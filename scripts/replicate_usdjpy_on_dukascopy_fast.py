...
from replicate_usdjpy_on_dukascopy import MAX_WORKERS, _hour_url, build_candidate_rows, decode_hour, load_h10_release_values, summarize
...
def first_at_or_after(ticks, timestamp: datetime):
    for tick in ticks:
        if tick.timestamp >= timestamp: return tick
    return None
...
def fetch_bucket(day: date, hour_utc: int):
    ...
    for attempt in range(RETRIES):
        try:
            ...
            if exc.code == 404: return day, [], url, "", "HTTP 404: historical bucket unavailable"
            if exc.code not in {429,500,502,503,504} or attempt + 1 == RETRIES: return day, [], url, "", f"HTTP {exc.code}: {exc.reason}"
        except (URLError, TimeoutError) as exc:
            if attempt + 1 == RETRIES: return day, [], url, "", str(exc)
        sleep(min(2 ** attempt, 8))
    return day, [], url, "", "unknown fetch failure"

def load_quotes(rows: list, latencies: list[int]):
    by_day = {r.release_date:r for r in rows}; days = market_days(rows); buckets = {}; errors = {}
    ...
    for lat in latencies:
        for day,(ticks,url,sha) in buckets.items():
            release=by_day[day].release_at; tick=first_at_or_after(ticks, release+timedelta(seconds=lat))
...
def execute(rows:list,quotes:dict,provenance:dict,slip_bps:Decimal,latency:int):
    ...
    for i,row in enumerate(rows):
        ...
        if row.signal>0:
            entry_px=entry.ask*(Decimal("1")+slip); exit_px=exit_tick.bid*(Decimal("1")-slip)
        else:
            entry_px=entry.bid*(Decimal("1")-slip); exit_px=exit_tick.ask*(Decimal("1")+slip)
        ret=exit_px/entry_px-Decimal("1") if row.signal>0 else entry_px/exit_px-Decimal("1")
