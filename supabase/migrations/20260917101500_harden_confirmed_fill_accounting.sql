...
--
-- Changes:
--   * commission/financing remain signed broker accounting values;
--   * duplicate fills are payload-checked, not merely identity-checked;
--   * realized price P&L is kept separate from financing and commission;
--   * the confirmed broker order id is persisted when the internal row was created
--     before broker acceptance and therefore had no ticket yet.

alter table public.execution_fills
  drop constraint if exists execution_fills_commission_nonnegative;

alter table public.execution_fills
  drop constraint if exists execution_fills_financing_nonnegative;
...
declare
  ...
  v_event_hash text;
begin
  if p_environment not in ('demo','paper','live') then
  ...
  if v_order.broker_order_id is not null and v_order.broker_order_id <> p_broker_order_id then
    raise exception 'broker_order_id mismatch';
  end if;
  ...
  where ef.fill_id = p_fill_id
     or (
       ef.broker_fill_id is not null
       and p_broker_fill_id is not null
       and ef.order_id = p_order_id
       and ef.broker_fill_id = p_broker_fill_id
     )
  limit 1;

  if found then
    if v_existing_fill.order_id <> p_order_id
       or v_existing_fill.broker_fill_id is distinct from p_broker_fill_id
       or v_existing_fill.filled_at <> p_filled_at
       or v_existing_fill.quantity <> p_quantity
       or v_existing_fill.price <> p_price
       or v_existing_fill.side <> v_db_side
       or v_existing_fill.venue is distinct from p_broker
       or v_existing_fill.commission <> p_commission
       or v_existing_fill.financing <> p_financing
       or coalesce(v_existing_fill.metadata,'{}'::jsonb) <> coalesce(p_metadata,'{}'::jsonb)
    then
      raise exception 'fill_id_conflict:%', p_fill_id;
    end if;
    ...
    return query select true,
      ...
      v_position.instrument,
      v_position.net_quantity,
      coalesce(v_position.average_price,0),
      v_position.realized_pnl,
      v_position.financing_pnl,
      v_position.state;
    return;
  end if;
  ...
  set broker_order_id = coalesce(eo.broker_order_id, p_broker_order_id),
  ...
  v_payload := jsonb_build_object(
    'fill_id',p_fill_id,
    'order_id',p_order_id,
    'broker_fill_id',p_broker_fill_id,
  ...
  where ee.environment=p_environment
    and ee.correlation_id=p_client_order_id
